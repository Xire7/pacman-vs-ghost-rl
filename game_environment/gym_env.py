...
from graphicsUtils import refresh
import ghostAgents  # Or your custom ghost
import graphicsDisplay, textDisplay
import time
...
class PacmanEnv(gym.Env):
    ...
    """
    def __init__(self, layout_name='mediumGrid', ghost_agents=None, max_steps=None, render_mode=None):
        super().__init__()
        ...
        if render_mode == 'human':
            ...
            pass # No render, fastest for training
    ...
    def reset(self, seed=None, options=None):
        ...
        print(f"\nReset: {self.game_state.getNumAgents()} agents initialized")
        pacman_state = self.game_state.data.agentStates[0]
        print(f"  Pacman: isPacman={pacman_state.isPacman}, pos={pacman_state.getPosition()}")
        

        # Initialize display on first reset
    ...
    def render(self, mode='human'):
        ...
        If render_mode was set in __init__, rendering happens automatically in step(). Otherwise, this method is called for manual rendering
        """
        if mode == 'human':
            if self.display is not None:
                # graphic
